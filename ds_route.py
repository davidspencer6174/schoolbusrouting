...
def trav_time(loc1, loc2):
    return ds_constants.TRAVEL_TIMES[loc1.tt_ind,
                                  loc2.tt_ind]
...
class ds_route:
    ...
    def __init__(self):
        ...
        self.max_time = ds_constants.MAX_TIME
    ...
    #and so all ds_routes must be reverted and the moved students
    #returned to the bus.    
    def backup(self):
    ...
    #Inserts a stop visit at position pos in the ds_route.
    ...
    def add_stop(self, stop, pos = -1):
        ...
        if pos == -1:
            ...
            self.max_time = max(self.max_time,
                                ds_constants.SLACK*trav_time(stop, stop.school))
            return True
        #Add the stop
    ...
    #Returns true if the insertion is valid w.r.t. ds_route length.
    ...
    def time_check(self, school_perm):
        ...
        time = 0
        mintime = school_perm[0].start_time - ds_constants.EARLIEST
        maxtime = school_perm[0].start_time - ds_constants.LATEST
        for i in range(1, len(school_perm)):
            ...
            #at the first school.
            if leg_time > 0:
                leg_time += ds_constants.SCHOOL_DROPOFF_TIME
            mintime += leg_time
            maxtime += leg_time
            time += leg_time
            school_mintime = school_perm[i].start_time - ds_constants.EARLIEST
            school_maxtime = school_perm[i].start_time - ds_constants.LATEST
        ...
        #if time > ds_constants.MAX_SCHOOL_DIST*1.5:
        #    memoized_timechecks[tuple(school_perm)] = (False, 0)
        #    return (False, 0)
    ...
    def recompute_length_naive(self):
        ...
        for i in range(0, len(self.schools) - 1):
            ...
            self.length += ds_constants.SCHOOL_DROPOFF_TIME
    ...
    def recompute_maxtime(self):
        self.max_time = ds_constants.MAX_TIME
        for stop in self.stops:
            self.max_time = max(self.max_time, ds_constants.SLACK*
                                            trav_time(stop, stop.school))
        
    #Determines whether the ds_route is feasible with
    ...
    def feasibility_check(self, verbose = False):
        ...
        for s in self.stops:
            self.max_time = max(self.max_time,
                                ds_constants.SLACK*trav_time(s, s.school))
        #Too long
        self.enumerate_school_orderings()
        self.recompute_length()
        ...
        for s in self.stops:
            for student in s.students:
                ...
                if e_found and h_found:
                    if verbose:
                        print("Elementary and high on same ds_route")
                    return False
    ...
    #Returns whether it is valid to assign the bus to the ds_route
    ...
    #stud_type to the ds_route given the bus capacity
        ...
        for i in range(len(self.stops)):
            ...
            while self.stops[i].school != self.schools[j]:
                ...
                if trav_time(self.schools[j], self.schools[j+1]) > 0.1:
                    this_stop_time += ds_constants.SCHOOL_DROPOFF_TIME
                j += 1
    ...
    #the total ds_route length within allowable bounds.

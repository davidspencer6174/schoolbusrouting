...
def obtainClust_DBSCAN_custom(schools_students_attend):
    indexes = schools_students_attend["tt_ind"].tolist()
    sub_travel_times = [[0 for x in range(0, len(indexes))] for y in range(0, len(indexes))] 
    
    for i, ind_1 in enumerate(indexes): 
        for j, ind_2 in enumerate(indexes):
            sub_travel_times[i][j] = constants.TRAVEL_TIMES[ind_1][ind_2]
